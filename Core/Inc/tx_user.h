 ...
 * though the compiler's equivalent of the -D option.
   ...
   For maximum speed, the following should be
 * defined:
 ...
 * TX_TIMER_PROCESS_IN_ISR
   ...
   Of course, many of these defines reduce functionality and/or change
 * the behavior of the
   system in ways that may not be worth the trade-off. For example, the
 * TX_TIMER_PROCESS_IN_ISR
   results in faster and smaller code, however, it increases the amount
 * of processing in the ISR.
   In addition, some services that are available in timers are not
 * available from ISRs and will
   therefore return an error if this option is used. This may or may
 * not be desirable for a
   given application.  */

/* Override various options with default values already assigned in tx_port.h. Please also refer
 to
 * tx_port.h for descriptions on each of these options.  */
...
/* Determine if timer expirations (application timers, timeouts, and tx_thread_sleep calls
   should
 * be processed within the a system timer thread or directly in the timer ISR.
   By default, the
 * timer thread is used. When the following is defined, the timer expiration
   processing is done
 * directly from the timer ISR, thereby eliminating the timer thread control
   block, stack, and
...
/* Determine is stack filling is enabled. By default, ThreadX stack filling is enabled,
   which
 * places an 0xEF pattern in each byte of each thread's stack.  This is used by
   debuggers with
 * ThreadX-awareness and by the ThreadX run-time stack checking feature.  */
 ...
 * checking is enabled (TX_ENABLE_STACK_CHECKING is defined), the TX_DISABLE_STACK_FILLING
   define
 * is negated, thereby forcing the stack fill which is necessary for the stack checking
   logic. */
...
/* Determine if global ThreadX variables should be cleared. If the compiler startup code clears
 the
 * .bss section prior to ThreadX running, the define can be used to eliminate unnecessary
 clearing
 * of ThreadX global variables.  */
 ...
 * Note: if TX_NO_TIMER is used, the define TX_TIMER_PROCESS_IN_ISR
   must also be used.  */
...
/* Determine if the tx_thread_resume and tx_thread_suspend services should have their internal
 code
 * in-line. This results in a larger image, but improves the performance of the thread
   resume and
 * suspend services.  */
...
/* Determine if the internal ThreadX code is non-interruptable. This results in smaller code
   size
 * and less processing overhead, but increases the interrupt lockout time.  */
 ...
 * code size and overhead, but provides the ability to generate system trace information which
   is
 * available for viewing in TraceX.  */

/*#define TX_ENABLE_EVENT_TRACE*/

/* Determine if block pool performance gathering is required by the application. When the following
 * is
...
/* Determine if event flags performance gathering is required by the application. When the following
 * is
...
/* Determine if semaphore performance gathering is required by the application. When the following
 * is
...
/* Determine if there is a FileX pointer in the thread control block.
   By default, the pointer is
 * there for legacy/backwards compatibility.
   The pointer must also be there for applications
 * using FileX.
