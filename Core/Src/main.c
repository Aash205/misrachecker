 ...
 ******************************************************************************
 * @file
 * ThreadX/Tx_Thread_Creation/Src/main.c
...
/* syscalls.c's _write() calls this with external linkage (see the
 * `extern int __io_putchar(int ch) __attribute__((weak));` there) --
 * declare it here too so the definition below has a visible declaration
 * in its own file. */
 ...
 * when TIM6 interrupt took place, inside
